...
//API GET request// NPM request package instead of https.get//

// const request = require('request');
// request("https://api.themoviedb.org/3/discover/movie?sort_by=popularity.desc?&api_key=f04fda10cc4d16ff8357bd2e7a47db2c", function(error,response,body){
...
https.get("https://api.themoviedb.org/3/discover/movie?sort_by=popularity.desc?&api_key=f04fda10cc4d16ff8357bd2e7a47db2c", function(response){
  let str = '';
  response.on('data', function(chunk){
      str += chunk;
  })
...
app.get('/movie/:movieId', (req, res) => {
  let list = getmovies();
    for (let i = 0; i < list.length; i++){
      if (list[i].id === req.params.movieId){
         res.render('pages/movie', {movie:list[i]});
      }
    }  
})

//trying for the search function//
app.get('/search/:movieTitle', (req, res)=> {
    let list = getmovies();
    const searchArray = [];
    for(let i = 0; i < list.length; i++){
      if(list[i].title.toLowerCase().includes(req.params.movieTitle.toLowerCase())){
          searchArray.push(list[i]);
      }
    }
    res.render('pages/search', {searchArray})
});
...
//giving express access to static files from the public folder//
